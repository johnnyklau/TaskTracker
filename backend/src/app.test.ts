...
describe('PATCH /tasks', () => {
  it('marks a created task as complete', async () => {
    ...
    const response = await request(app).patch(`/tasks/${insertResponse.rows[0].id}`).send({ completed: true });
    expect(response.status).toBe(200)
...
describe('DELETE /tasks/:id', () => {
  it('deletes a task', async () => {
    ...
    const response = await request(app).delete(`/tasks/${insertResponse.rows[0].id}`);
